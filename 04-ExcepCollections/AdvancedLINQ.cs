    ...
    class Program
    {
        ...
        new Statistic { CharacterId = 5, Power = 95 }
            ...
            foreach (var team in teamsMethod)
            {
                ...
                {
                    WriteLine($"Personaje: {character.Name}");
                }
            }
            ...
            foreach (var c in charactersWithAbilities)
            {
                WriteLine($"Personaje y habilidades: {c.Name} {c.Description}");
            }

            //AGREGACIONES
            var totalPower = statistics.Sum(s => s.Power);
            ...
            {
                WriteLine($"Character: {item.Character} , Abitilites: {item.Count}");
            }
        }
    }
}